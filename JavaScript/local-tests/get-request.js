...
async function getRequest() {
  ...
  xhr.onload = function () {
    ...
    if (API_response.length !== undefined && xhr.status == 200) {
      ...
      console.log(
        "- Failed: 200 response failed from the API, instead got:",
        xhr.status,
      );
    }
    ...
    if (API_response.length !== undefined) {
      ...
      console.log(
        `- Passed: Number of ID's from API response, total ID's: ${mockData.length}/${idCount}`,
      );
    } else {
    ...
    if (
      API_response.length !== undefined &&
      Object.keys(API_id15).length == 7
    ) {
      ...
      for (const key of levelTwoKeys) {
        assert.equal(mockData_id15.rating[key], API_id15.rating[key]);
      }

      console.log(
        "- Passed: Validated all items and content of product with ID 15, total items:",
        Object.keys(API_id15).length,
      );
    } else {
      console.log(
        "- Failed: Validate all items and content of product with ID 15",
      );
    }
  };
